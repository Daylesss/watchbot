...
def parse_order(username: str, order: tuple):
    logging.info(f"Parsing order %r %s", username, order)
    if order[0]=="book":
        book = "ЗАБРОНИРОВАЛ"
    else:
        book = "КУПИЛ"
...
def parse_wrong_order(username: str, order: tuple, is_higher: bool):
    logging.info(f"Parsing WRONG order %s %s %s", username, order, is_higher)
    if order[0]=="book":
        book = "БРОНИРОВАНИЯ"
    else:
        book = "ПОКУПКИ"
...
async def send_qr(call: types.CallbackQuery, state: FSMContext, bot: Bot, data:dict):
    logging.info(f"Sending qr: %s", data)
    ...
    if not(is_for_sale):
        logging.info(f"%s already booking")
        await state.clear()
    ...
    logging.info(f"Creating qr %s", watch_id)
    ...
    is_higher = False
    is_lower = False
    logging.info(f"Polling for watch: %s user: %s", watch_id, call.from_user.id)
    for i in range(60):
    ...
    logging.info(f"Poling ended watch: %s user: %s %s %s %s", watch_id, call.from_user.id, is_bought, is_higher, is_lower)
    os.remove(qr_name)
    if not(is_bought):
        await qr.delete()
        ...
        logging.info(f"Reset all data for watch: %s", watch_id)
        await state.clear()
        if not is_lower:
            return
    ...
    logging.info(f"watch, trans, files %s %s %s", watch_id, trans_data, files)
    if is_lower:
        logging.info(f"Is lower watch")
        await qr.delete()
...
async def get_book2(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    logging.info("Get book2: %s %s", call.data, call.from_user.id)
    watch_id = await get_watch_id(call.from_user.id)
    files = await get_files(watch_id)
    if type(files)==type(" "):
...
async def no_buy(call: types.CallbackQuery, state: FSMContext):
    logging.info(f"{call.from_user.id} no_buy")
    try:   
...
async def yes_buy(call: types.CallbackQuery, state: FSMContext):
    logging.info(f"Watch: %s %s", call.data, call.from_user.id)
    try:   
        await call.message.edit_reply_markup(reply_markup=None)
    except:
        print("не удалось удалить кнопки")
...
async def address(message: types.Message, state: FSMContext):
    logging.info(f"Watch addres: %s %s", message.text, message.from_user.id)
...
def make_hash(data: dict, tg_id: int):
    logging.info("Making hash %s %s", tg_id, data)
    hash_dict2 = {
...
async def cancel_book(call: types.CallbackQuery, state: FSMContext):
    logging.info(f"Canceling book %s", call.from_user.id)
    try:   
        await call.message.edit_reply_markup(reply_markup=None)
    except:
        print("не удалось удалить кнопки")
...
async def address(call: types.CallbackQuery, state: FSMContext, bot: Bot):
    logging.info(f"Continue booking %s", call.from_user.id)
    try:   
        await call.message.edit_reply_markup(reply_markup=None)
    except:
        print("не удалось удалить кнопки")
